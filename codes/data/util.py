...
def read_img(env, path, size=None):
    ...
    if img.shape[2] > 3:
        img = img[:, :, :3]
    return img
def read_img_for_yuv(env, path, size=None):
    """read image by cv2 or from lmdb
    return: Numpy uint32, HWC, BGR, [0-255]"""
    if env is None:  # img
        img = cv2.imread(path, cv2.IMREAD_UNCHANGED)
    else:
        img = _read_img_lmdb(env, path, size)
...
def argment_rgb(im1, im2, prob=1.0):
...
def argment_cutblur(im1, im2, scale, prob=1.0, alpha=0.2):
    """
    ...
    im1 = imresize_np(im1, 1 / scale)

    im2 = cv2.rectangle(im2, (cy, cx), (cy + ch, cx + cw), (0, 0, 255), 5)
    if im1.shape[0] * scale != im2.shape[0] or im1.shape[1] * scale != im2.shape[1]:
        raise ValueError("im1 and im2 have to be the same resolution.")
    return im1, im2
